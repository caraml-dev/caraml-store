...
public class JobService {
  ...
  private final Map<JobType, Map<String, SparkJobConfig>> jobConfigByTypeAndStoreName =
      new HashMap<>();
  ...
  public JobService(
    ...
    this.jobConfigByTypeAndStoreName.put(
        ...
        config.streamingJobs().stream()
            .collect(Collectors.toMap(SparkJobConfig::store, Function.identity())));
  ...
  private void createOrUpdateIngestionJob(JobType jobType, String project, FeatureTableSpec spec)
    ...
    Map<String, SparkJobConfig> jobConfigByStoreName = jobConfigByTypeAndStoreName.get(jobType);
    ...
    SparkJobConfig jobProperties = jobConfigByStoreName.get(spec.getOnlineStore().getName());
